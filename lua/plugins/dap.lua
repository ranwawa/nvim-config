...
local function setup_dap()
  ...
  map("n", "<leader>dh", function()
    dap.set_breakpoint(nil, vim.fn.input("命中次数（第 N 次才停）: "))
  end, { desc = "DAP 命中次数断点" })
  map("n", "<leader>dL", function()
    dap.set_breakpoint(nil, nil, vim.fn.input("Logpoint 输出模板: "))
  end, { desc = "DAP Logpoint（不暂停，只打印）" })
  ...
  map("n", "<leader>dp", function()
    dap.pause()
  end, { desc = "DAP 暂停正在运行的进程" })
  map("n", "<leader>dD", function()
    dap.disconnect({ terminateDebuggee = false })
  end, { desc = "DAP 脱离调试但保留进程" })
  -- 调用栈切换帧（等价于在 Stacks 窗口里上下选帧）
  map("n", "<leader>dj", dap.down, { desc = "DAP 切到下一层帧" })
  map("n", "<leader>dk", dap.up, { desc = "DAP 切到上一层帧" })
